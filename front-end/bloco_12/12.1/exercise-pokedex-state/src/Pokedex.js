  ...
  handleType(event) {
    ...
    this.setState({
      pokemons: pokemons.filter(
        (pokemon) => filter === "All" || pokemon.type === filter
      ),
      index: 0,
    });
  }
  ...
  render() {
    ...
    return (
      <>
        ...
        <Button onClick={this.handleNext} disabled={pokemons.length < 2}>
          NEXT
        </Button>

        <Button key="all" onClick={this.handleType}>
          All
        </Button>
        {this.types.map((type) => (
          <Button key={type} onClick={this.handleType}>
            {type}
          </Button>
        ))}
      </>
